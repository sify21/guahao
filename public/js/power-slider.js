...
//用来控制快速预约模块的逻辑关系
function ul1click() {
	$("#_ul1").find("li").click(function () {
		if ($(this).html() == "请选择") {
			$("#FastdropdownMenu1").html("请选择<span class='caret'></span>");
			$("#FastdropdownMenu2").html("请选择<span class='caret'></span>").addClass("disabled");
			$("#FastdropdownMenu2").attr({xid:-1});
			$("#FastdropdownMenu3").html("请选择<span class='caret'></span>").addClass("disabled");
			$("#FastdropdownMenu3").attr({xid:-1});
			$("#FastdropdownMenu4").html("请选择<span class='caret'></span>").addClass("disabled");
			$("#FastdropdownMenu4").attr({xid:-1});
		}
		else {
			if ($(this).html() != $("#FastdropdownMenu2").html()) {
				$("#FastdropdownMenu1").html($(this).html());
				$("#FastdropdownMenu2").html("不限<span class='caret'></span>").removeClass("disabled");
				$("#FastdropdownMenu2").attr({xid:0});
				$("#FastdropdownMenu3").html("请选择<span class='caret'></span>").removeClass("disabled");
				$("#FastdropdownMenu3").attr({xid:-1});
				$("#FastdropdownMenu4").html("请选择<span class='caret'></span>").addClass("disabled");
				$("#FastdropdownMenu4").attr({xid:-1});
				$.post("/index/province", {"province": $(this).html()}, function(data){refreshCityHos(data);}, "JSON");
			}
		}
	})
}
function ul2click() {
	$("#_ul2").find("li").click(function () {
		if ($(this).html() == "不限") {
			$("#FastdropdownMenu2").html("不限<span class='caret'></span>");
			$("#FastdropdownMenu2").attr({xid:0});
			$.post("/index/city", {"cityid": "0"}, function (city) {refreshHos(city);}, "JSON");
		}
		else {
			var cityxid=$(this).attr("xid");
			if ($(this).html() != $("#FastdropdownMenu2").html()) {
				$("#FastdropdownMenu2").html($(this).html());
				$("#FastdropdownMenu2").attr({xid: $(this).attr("xid")});
				$("#FastdropdownMenu3").html("请选择<span class='caret'></span>").removeClass("disabled");
				$("#FastdropdownMenu3").attr({xid:-1});
				$("#FastdropdownMenu4").html("请选择<span class='caret'></span>").addClass("disabled");
				$("#FastdropdownMenu4").attr({xid:-1});
				$.post("/index/city", {"cityid": $(this).attr("xid") }, function (city) {console.log(city);refreshHos(city);}, "JSON");
			}
		}
	})
}
function ul3click(){
	$("#_ul3").find("li").click(function () {
		($(this).html() != $("#FastdropdownMenu3").html())
		$("#FastdropdownMenu3").html($(this).html());
		$("#FastdropdownMenu3").attr({xid: $(this).attr('xid')});
		$("#FastdropdownMenu4").html("请选择<span class='caret'></span>").removeClass("disabled");
		$("#FastdropdownMenu4").attr({xid:-1});
		$.post("/index/hospital", {"hospitalid": $(this).attr('xid')}, function (hospital) {refreshDepart(hospital);}, "JSON");
	})
}
function ul4click() {
    $("#_ul4").find("li").click(function () {
        $("#FastdropdownMenu4").html($(this).html());
        $("#FastdropdownMenu4").attr({xid: $(this).attr('xid')});
    })
}

//添加四个额外的函数来发送post请求更新下拉列表数据
function refreshCityHos(data){
	$("#_ul2 li").unbind();
	$("#_ul2").empty();
    $("#_ul2").append("<li xid='0' role='menuitem'>不限</li>");
    for(i in data.city){
        $("#_ul2").append("<li xid='" +i+"' role='menuitem'>"+ data.city[i] + "</li>");
    }
	ul2click();
	$("#_ul3 li").unbind();
	$("#_ul3").empty();
    for(i in data.hospital){
        $("#_ul3").append("<li xid='" +i+"' role='menuitem'>"+ data.hospital[i] + "</li>");
    }
	ul3click();
}
...
function refreshDepart(data){
	$("#_ul4 li").unbind();
	$("#_ul4").empty();
    for(i in data.department){
        $("#_ul4").append("<li xid='" +i+"' role='menuitem'>"+ data.department[i] + "</li>");
    }
	ul4click();
}
